...
public class ParticleSphere {
	...
	public void setup() {
		  ...
		  // set begining p.color of frames
		  ...
		  particles = new Particle[(p.width + particleMargin * 2) / particlesDensity * 
		  ...
		  for(int y=-particleMargin; y<p.height+particleMargin; y+=particlesDensity) { 
		    for(int x=-particleMargin; x<p.width+particleMargin; x+=particlesDensity) { 
		       ...
		       int c = p.color(50+50*PApplet.sin(PConstants.PI*x/p.width), 127, 255*PApplet.sin(PConstants.PI*y/p.width));
		       particles[i++] = new Particle(p, this, c, (int)p.random(64), (int)p.random(64), (int)p.random(64), theta, u); 
		    } 
		  }
		}
	...
	public void draw() {
		  ...
		  //if(pixMode == 1 && moveMode == 1 && fileName != "start.txt") imgProc.scaleBrightness(tempFrame, tempFrame, p.width, p.height, 0.2);
		  
		  PApplet.arrayCopy(tempFrame, currFrame);
		  radius = startingRadius + centerX-p.mouseX/2; 
